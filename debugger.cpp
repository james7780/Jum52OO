// Debugger code for Jum52 emulator
// Copyright James Higgs 2018
...
extern C5200 jum52;
extern void clrEmuScreen(unsigned char colour);
extern void printXY(char *s, int x, int y, unsigned int colour);
extern void BlitBuffer(int destX, int destY);
extern char getkey(void);
extern void showPMdebug(void);
extern void display_charset(void);
...
// Get name of HW register, given it's address
...
void hexview(uint16 viewaddr) {
	...
	memory = memory5200;
	clrEmuScreen(0x00);
	for (int i = viewaddr; i < viewaddr + 160; i++) {
		if ((count % 8) == 0) {
			sprintf(msg, "%4X: ", i);
...
// Get name of instruction, given it's opcode 
static void show_opcode(char *mystring, uint8 instr)
{
	switch (instr) {
		case 0x6d:
...
static void show_operand(char *mystring, uint8 instr)
{
	...
	switch (instr) {
		...
		case 0x70:					/* BVS */
			byte = memory[addr];
			addr++;
			sprintf(mystring, "$%x", addr + (char)byte);
			break;
			/*
				 =========================
...
void showcollisionregs()
{
	...
	for (i = 0; i < 4; i++)
		{
		sprintbin(s, jum52.M2PF[i]);
		sprintf(msg, "M%dPF %s", i, s);
		printXY(msg, 0, i * 8, 255);
		sprintbin(s, jum52.M2PL[i]);
	...
	for (i = 0; i < 4; i++)
		{
		sprintbin(s, jum52.P2PF[i]);
		sprintf(msg, "P%dPF %s", i, s);
		printXY(msg, 0, i * 8 + 64, 255);
		sprintbin(s, jum52.P2PL[i]);
